  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: state.cart.isEmpty
          ? const _EmptyCart()
          : _CartContent(state: state, promoController: _promoController),
      bottomNavigationBar: state.cart.isEmpty
          ...
          : SafeArea(
              ...
              child: FilledButton(
                ...
                child: Text(
                    'Proceed to checkout - ${currency.format(state.total)}'),
              ),
            ),
    );
  }
        ...
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.shopping_bag_outlined,
                size: 76, color: Colors.grey.shade400),
            const SizedBox(height: 16),
            Text('Your cart is empty',
                style: Theme.of(context).textTheme.titleLarge),
            const SizedBox(height: 6),
            const Text('Add something delicious from the menu.',
...
class _CartContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(
        ...
        children: [
          ...
          Card(
            child: Padding(
              ...
              child: Column(
                  ...
                  children: [
                    OrderTypeToggle(
                        value: state.orderType, onChanged: state.setOrderType),
                    ...
                    Container(
                      ...
                      child: const Row(children: [
                        ...
                        Expanded(
                          child: Text(
                            'If you have a food allergy, phone the store before ordering.',
          ...
          Wrap(
              spacing: 8,
              children: <double>[0, .10, .15, .20]
                  .map((tip) => ChoiceChip(
                      label:
                          Text(tip == 0 ? 'No tip' : '${(tip * 100).round()}%'),
                      selected: state.tipRate == tip,
                      onSelected: (_) => state.setTip(tip)))
                  .toList()),
          const SizedBox(height: 18),
          Card(
            color: Colors.white,
            child: Padding(
              ...
              child: Column(children: [
                ...
                if (state.discount > 0)
                  PriceRow(
                      'Promo discount', '-${currency.format(state.discount)}'),
  ...
  String get _destination {
    if (state.orderType == OrderType.delivery) {
      return state.location.isEmpty ? 'Address required' : state.location;
    }
    return state.selectedStore?.address ?? 'Branch selection required';
  }
  ...
  Future<void> _applyPromo(BuildContext context) async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(valid
            ...
            : state.promoMessage ?? 'That promo code is not valid.')));
  }
}
          ...
          Column(children: [
            Row(mainAxisSize: MainAxisSize.min, children: [
              IconButton(
                  ...
                  onPressed: () => state.quantity(item.id, item.quantity - 1),
            ...
            TextButton(
                onPressed: () => state.remove(item.id),
